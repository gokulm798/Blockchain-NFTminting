...
const reqAcceptLicense= asyncHandler(async (req, res) => {
    ...
    const {reqId}=req.body
    const change = await licenseRequest.findByIdAndUpdate(reqId ,{ $set: { isUserRead:true,isAccept:true} });
...
const reqDeclineLicense = asyncHandler(async (req, res) => {
    ...
    const {reqId}=req.body
    const change = await licenseRequest.findByIdAndUpdate( reqId,{ $set: { isUserRead:true,isAccept:false} });
...
module.exports = { userRequest,reqCheck,reqAccept,reqDecline,senderReq,userRequestLicense,reqCheckLicense,reqAcceptLicense,reqDeclineLicense,senderReqLicense};